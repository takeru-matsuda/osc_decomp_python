...
def osc_decomp_uni(
    ...
    Args:
        ...
        MAX_AR: int Defaults to None.
            ...
            set to be max(20, 2 * MAX_OSC) when None (default).
    ...
    Returns:
        osc_param: ndarray, shape = (MAX_OSC, 3 * MAX_OSC + 1)
            ...
            osc_param[K - 1, 3*K] is the estimated tau^2 of 
            the oscillator model with K oscillation components
    ...
    for K in range(1, MAX_OSC + 1):
        ARdeg = np.nanargmin(ARwithnoise_AIC) + 1
        tmp = np.roots(
            np.concatenate([
                np.ones(1), 
